...
enum RecipeListFilterSupport {
    ...
    /// by relevance and the list drops the browse sort. DSL filter tokens
    /// typed into the search field (`tag:`/`source:`/`created:`/`photo_*:`/
    /// `has:`/`no:`) parse as filters, not text, so a filter-only search
    /// still browses by the chosen sort. Structured filters (tag chips,
    /// source, dates) live outside searchText and are likewise not text
    /// terms.
    static func hasTextQuery(_ state: RecipeListFilterState) -> Bool {
        !RecipeSearchSupport.parse(state.searchText).textTokens.isEmpty
    }

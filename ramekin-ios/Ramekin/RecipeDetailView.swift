...
extension RecipeDetailView {
    ...
    func formatIngredient(_ ingredient: Ingredient) -> String {
        ...
        if ingredient.measurements.count > 1 {
            let altTexts = ingredient.measurements.dropFirst().compactMap { measurement -> String? in
                let altParts = [measurement.amount, measurement.unit].compactMap { $0 }.filter { !$0.isEmpty }

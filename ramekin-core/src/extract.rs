...
fn scan_substack_blocks(body: ElementRef<'_>) -> Vec<SubstackBlock> {
...
fn substack_element_text(elem: ElementRef<'_>) -> String {
...
fn collect_substack_text(elem: ElementRef<'_>, buf: &mut String) {
...
fn is_substack_widget_element(elem: ElementRef<'_>) -> bool {
    elem.value()
        .attr("class")
        .is_some_and(|c| c.contains("subscribe-widget"))
}

fn is_instructions_marker(text: &str) -> bool {

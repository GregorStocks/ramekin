...
pub async fn run_enrich_test(config: EnrichTestConfig) -> Result<EnrichTestResults> {
    // Generate run ID
    let now = Utc::now();
    ...
    // Initialize results
    let mut results = EnrichTestResults {
...
fn generate_report(results: &EnrichTestResults) -> String {

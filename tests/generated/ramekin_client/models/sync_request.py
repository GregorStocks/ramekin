...
class SyncRequest(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in creates (list)
        _items = []
        if self.creates:
            for _item_creates in self.creates:
                if _item_creates:
                    _items.append(_item_creates.to_dict())
            _dict['creates'] = _items

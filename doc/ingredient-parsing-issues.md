...
- [x] **Standalone continuation lines starting with "and", "or", or "plus"** (~20 lines in ingredient-categories.csv) - Examples like "and 2 Tbsp sugar", "or 2 regular carrots", "plus 1 tablespoon extra-virgin olive oil". These likely belong to the previous ingredient as an alternative/addition but currently parse as orphaned lines. **FIXED:** Added `strip_leading_continuation()` in `parse_ingredients()` to strip leading "and ", "or ", "plus " (case-insensitive) before passing to `parse_ingredient()`. This converts "Or Spectrum coconut oil" → "Spectrum coconut oil". Note: Many items in ingredient-categories.csv that look like continuations (e.g., "and 2 Tbsp sugar") are actually mid-line parsing failures from inputs like "1/2 cup and 2 Tbsp sugar", which is a separate issue. Only 2 pipeline fixtures had actual standalone continuation lines. The remaining ~19 items are from the mid-line "and" parsing issue.
...
**2026-02-03 (Claude Opus 4.5)** - Fixed standalone continuation lines! Added `strip_leading_continuation()` in `parse_ingredients()` to strip "and ", "or ", "plus " prefixes before parsing. The interesting discovery: most items in ingredient-categories.csv that look like continuations (e.g., "and 2 Tbsp sugar") aren't actually standalone lines - they're mid-line parsing failures from inputs like "1/2 cup and 2 Tbsp sugar" where the parser extracts "1/2 cup" and leaves "and 2 Tbsp sugar" as the item. Only 2 pipeline fixtures had true standalone continuation lines (sprinklebakes with "Or Spectrum coconut oil"). The ~19 other continuation-looking items need a different fix: better handling of "and" within lines, which is a more complex problem. Future Claude: if you tackle mid-line "and" handling, note that "1 and 1/2 cups" (mixed numbers) already works - the issue is "1/2 cup and 2 Tbsp" (two separate amounts in one line).

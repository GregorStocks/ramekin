...
- [x] **Double-encoded HTML entities** - "&amp;#8531;" (double-encoded 1/3 fraction) now decodes correctly to "1/3". Fixed by decoding `&amp;` to `&` FIRST in `decode_html_entities()`, then adding numeric entity support for fraction characters (&#8531;=⅓, &#8532;=⅔) and other common entities. 69 fixtures updated.

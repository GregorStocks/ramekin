...
| Tag hierarchy | `ramekin-ui/src/utils/tagHierarchy.ts` | `ramekin-ios/Shared/TagHierarchySupport.swift` | `ramekin-core/src/tags.rs` (yes — validation lives here) | ~100–200 | Subtle — group sort uses `localeCompare` on raw names (web) vs `localizedCaseInsensitiveCompare` on parsed values (iOS); namespace-list ordering differs (`knownNamespaces` sorts seeded+discovered together, iOS keeps seeded order first). Seeded-namespace list is hardcoded in all three places. |
